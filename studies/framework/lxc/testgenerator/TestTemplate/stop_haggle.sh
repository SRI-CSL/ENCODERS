...
if [ "1" != "${RUNNING}" ]; then
    ...
    RSLEEP=`echo "scale=5;$RANDOM/32767" | bc`
    sleep $RSLEEP
    /usr/local/bin/haggletest -x nop &> /dev/null
    CODE=$?
    if [ "0" != "$CODE" ]; then
        #echo "${NODE_NAME}: Could not use haggletest to shutdown server - error code ${CODE}" >> ${FAILLOG}
        RSLEEP=`echo "scale=5;$RANDOM/32767*10+10" | bc`
        sleep $RSLEEP
        /usr/local/bin/haggletest -x nop &> /dev/null
        CODE=$?
        if [ "0" != "$CODE" ]; then
            #echo "${NODE_NAME}: Could not use haggletest to shutdown server try 2 - error code ${CODE}" >> ${FAILLOG}
            RSLEEP=`echo "scale=5;$RANDOM/32767*10+20" | bc`
            sleep $RSLEEP
            /usr/local/bin/haggletest -x nop &> /dev/null
            CODE=$?
            if [ "0" != "$CODE" ]; then
                echo "${NODE_NAME}: Could not use haggletest to shutdown server try 3 - error code ${CODE}" >> ${FAILLOG}
                kill -INT ${PID}
                if [ "0" != "$?" ]; then
