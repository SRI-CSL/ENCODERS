...
def generate_startup_file(template_file, startup_file, files):
    dd_template_string = "dd if=/dev/urandom of=%s bs=1024 count=%s &> /dev/null\n"
    push_template_string = "adb_n $NUM_DEVICES \"push %s %s%s\" &> /dev/null\n"
    dd_string = ""
    push_string = ""

    for f_tuple in files:
        dd_string = dd_string + (dd_template_string % (f_tuple[0], f_tuple[1]))
        push_string = push_string + (push_template_string % (f_tuple[0], TEST_DIR_PREFIX, f_tuple[0]))
    replace_file(template_file, startup_file, [ ('%%dd_string%%',  dd_string), ('%%push_string%%', push_string) ])
    chmodx(startup_file)
...
def create_security_configs(cfg, num_nodes):
    if not os.path.exists('/tmp/device_ids.txt'):
        raise Exception("Can not create security configuration without having a device list!")
    IDTable = []
    with open('/tmp/device_ids.txt') as input_file:
        for line in input_file.readlines():
            IDTable.append(hashlib.sha1(line.replace('\n', '')).hexdigest())
...
def create_test(        \
    ...
    test_output_path = test_output_path.replace("%%pretty%%", str(pretty_print_name)).replace("Testsuite", "AndroidTestsuite")
    ...
    replace_file(test_template + 'echo_num_devices.sh', test_output_path + 'echo_num_devices.sh', [ ( '%%num_devices%%', num_nodes) ])
    chmodx(test_output_path + 'echo_num_devices.sh')
    ...
    cp(test_template + 'start_haggle_onAndroid.sh', test_output_path + 'start_haggle_onAndroid.sh')
    ...
    generate_startup_file(test_template + 'start_up.sh', test_output_path + 'start_up.sh', files)
