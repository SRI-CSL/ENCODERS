...
# turn on tethering with USB (need com.saic.tetherusb)
turn_on_tether_usb () {
  idLen=${#id_list[@]}
  for ((j=0; j < idLen; j++)); do
   echo "Turn on tethering with USB: " "${id_list[j]}"
   adb_retry 20 ${id_list[j]} shell am start -a android.intent.action.Main -n com.saic.tetherusb/.TetherUSBActivity
   sleep 2
  done
  sleep 10
  echo ""
...
set_ip_address ()
{
  ...
  for ((i=0; i < idLen; i++)); do
    ip=$pip$d
    adb_retry 10 ${id_list[i]} shell busybox ifconfig usb0 $ip
    adb_retry 10 ${id_list[i]} shell ip route add default dev usb0
