...
for CAT in $CATS; do
TYPE=$(echo $CAT | cut -d'-' -f10)
SEED=s$(echo $CAT | cut -d'-' -f8 | cut -d 's' -f 2)
echo "\"delays_${SEED}_${TYPE}_a2a3.dat\" using 1:2 with linespoints ls ${I} title '${TYPE}', \\" >> delays_a2a3.plot
cat $CAT/$TYPE.apps_output | grep "Received," | egrep -v "1024,\(null\)" | egrep -v "5120,\(null\)" | egrep -v "10240,\(null\)" | awk -F, '{print $6;}' | sort -n | awk '{print $1, NR;}' > delays_${SEED}_${TYPE}_a2a3.dat
...
mv transport-cdf.eps ${BASE_NAME}-transport-cdf-${SEED}_a2a3.eps
