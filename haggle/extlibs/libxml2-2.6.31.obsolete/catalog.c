...
xmlInitializeCatalog(void) {
    ...
    {
		void* hmodule;
		hmodule = GetModuleHandleA(L"libxml2.dll");
		if (hmodule == NULL)
		...
		if (hmodule != NULL) {
			...
			if (len != 0) {
				...
				if (p != buf) {
					...
					wcsncpy(p, L"\\..\\etc\\catalog", 255 - (p - buf));

					WideCharToMultiByte(CP_UTF8, WC_NO_BEST_FIT_CHARS, buf, -1, buf2, 256, NULL, &flag);

					uri = xmlCanonicPath(buf2);
					if (uri != NULL) {
