...
typedef struct {
    ...
    char *duration;
    char *logfile;
    char *reduce_dist_frequency_after;
    char *set_max_hop_count_after;
    char *set_max_hop_count_after_average;
    /* special */
...
"\n"
"Usage:\n"
"  haggleobserver [--logfile=<logfile>] [--duration=<duration>] [--attribute_name=<attribute_name>] [--attribute_value=<attribute_value>] [--reduce-dist-frequency-after=<time>] [--set-max-hop-count-after=<count>] [--set-max-hop-count-after-average=<count>]\n"
"  haggleobserver --help\n"
"  haggleobserver --version\n"
"\n"
"Options:\n"
"  -h --help                             Show this screen.\n"
"  --version                             Show version.\n"
" --logfile=<logfile>                    Log File Path [default: haggleobserver.log].\n"
" --duration=<duration>                  Duration in seconds [default: 30].\n"
" --attribute_name=<attribute_name>      Attribute to subscribe to [default: ObserverDataObject].\n"
" --attribute_value=<attribute_value>    Attribute to subscribe to [default: true].\n"
" --reduce-dist-frequency-after=<time>   Reduce the frequency of observer data object distribution after time [default: -1].\n"
" --set-max-hop-count-after=<count>      Set the max hop count of Observer data objects to 1 after the number of one hop neighbors exceeds a threshold [default: -1].\n"
" --set-max-hop-count-after-averag=<count>  Set the max hop count of Observer data objects to 1 after the average number of one hop neighbors (taken over all nodes) exceeds a threshold [default: -1].\n"
"\n"
"";
...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_options; i++) {
        ...
        } else if (!strcmp(option->olong, "--attribute_name")) {
            if (option->argument)
                args->attribute_name = option->argument;
        } else if (!strcmp(option->olong, "--attribute_value")) {
            if (option->argument)
                args->attribute_value = option->argument;
        } else if (!strcmp(option->olong, "--duration")) {
            if (option->argument)
                args->duration = option->argument;
        } else if (!strcmp(option->olong, "--logfile")) {
            if (option->argument)
                args->logfile = option->argument;
        } else if (!strcmp(option->olong, "--reduce-dist-frequency-after")) {
            if (option->argument)
                args->reduce_dist_frequency_after = option->argument;
        } else if (!strcmp(option->olong, "--set-max-hop-count-after")) {
            if (option->argument)
                args->set_max_hop_count_after = option->argument;
        } else if (!strcmp(option->olong, "--set-max-hop-count-after-average")) {
            if (option->argument)
                args->set_max_hop_count_after_average = option->argument;
        }
    }
    /* commands */
