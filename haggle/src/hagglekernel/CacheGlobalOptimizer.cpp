...
CacheGlobalOptimizer::CacheGlobalOptimizer(
...
CacheGlobalOptimizerFixedWeights::CacheGlobalOptimizerFixedWeights(
    HaggleKernel *_kernel) :
        CacheGlobalOptimizer(_kernel, CACHE_GLOBAL_OPT_FIXED_WEGHTS_NAME),
        minThreshold(CACHE_GLOBAL_OPT_DEFAULT_THRESHOLD)
{
}

double
CacheGlobalOptimizerFixedWeights::getMinimumThreshold()
{
    return minThreshold;
}

double 
CacheGlobalOptimizerFixedWeights::getWeightForName(string name)
{
...
void 
CacheGlobalOptimizerFixedWeights::onConfig(const Metadata& m)
{
    const char *param;
    const Metadata *dm;
...
string
CacheGlobalOptimizerFixedWeights::getPrettyName()
{
    string pretty;
