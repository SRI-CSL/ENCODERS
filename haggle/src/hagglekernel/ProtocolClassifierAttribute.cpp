...
ProtocolClassifierAttribute::ProtocolClassifierAttribute(
    ProtocolManager *m)  :
    ProtocolClassifier(m, PROTOCOL_CLASSIFIER_ATTRIBUTE_NAME), 
    initialized(false),
...
void
ProtocolClassifierAttribute::onProtocolClassifierConfig(
    const Metadata& m)
{
    initialized = false;
    ...
    const char *_attrName = m.getParameter(PROTOCOL_CLASSIFIER_ATTRIBUTE_NAME_OPTION);
    if (!_attrName) {
        HAGGLE_ERR("No attribute name specified.\n");
        return;
    }    

    const char *_attrValue = m.getParameter(PROTOCOL_CLASSIFIER_ATTRIBUTE_VALUE_OPTION);
    if (!_attrName) {
    ...
    const char *_className = m.getParameter(PROTOCOL_CLASSIFIER_ATTRIBUTE_CLASS_NAME);
    if (!_className) {
    ...
    HAGGLE_DBG("Initialized protocol classifier with attr name: %s, attr value: %s, output tag: %s.\n",
        attrName.c_str(),
        attrValue.c_str(),
        className.c_str());
