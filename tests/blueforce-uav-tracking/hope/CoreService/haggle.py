...
class HaggleService(CoreService):
    ...
    def generateconfig(cls, node, filename, services):
        ...
        for ifc in node.netifs():
            ...
            break
        cfg += "/sbin/ifconfig eth0 broadcast 10.0.255.255\n"
        cfg += "/sbin/ifconfig eth1 broadcast 10.1.255.255\n"
        cfg += "/sbin/route add default eth0\n"
        cfg += "cp /tmp/config.xml ~/.Haggle/config.xml\n" 
        cfg += "/bin/su - %s -c \"/tmp/haggle -dd -f &\"\n" % (USER,)
        cfg += "/bin/su - %s -c \"%s %s %s &\"\n" % (USER, TESTAPP, node.name, OUTPUTFILE)
        cfg += "/bin/su - %s -c \"sleep 2610 && kill -INT $(cat ~/.Haggle/haggle.pid)\"\n" % (USER,)
        # make sure haggle is killed for good
        cfg += "/bin/su - %s -c \"sleep 30 && kill $(cat ~/.Haggle/haggle.pid)\"\n" % (USER,)
        return cfg
