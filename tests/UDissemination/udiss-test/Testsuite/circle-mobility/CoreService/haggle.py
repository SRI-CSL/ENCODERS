...
class HaggleService(CoreService):
    ...
    def generateconfig(cls, node, filename, services):
        ...
        # cfg += "/bin/su %s -l -c \"export LD_LIBRARY_PATH='/usr/local/lib/' && sleep 10 && bash %s %s %s %s &\"\n" % (USER, TESTAPP, node.name, OUTPUTFILE + "." + node.
        # cfg += "/bin/su %s -l -c \"export LD_LIBRARY_PATH='/usr/local/lib/' && sleep 10 && bash %s/start_resource_monitor.sh &\"\n" % (USER, TESTDIR)
        # cfg += "/bin/su %s -l -c \"export LD_LIBRARY_PATH='/usr/local/lib/' && sleep 10 && bash %s/start_neighbor_monitor.sh %s &\"\n" % ("root", TESTDIR, USER)
        ...
        cfg += "/bin/su %s -c \"sleep 10 && bash %s %s %s %s &\" - \n" % (USER, TESTAPP, node.name, OUTPUTFILE + "." + node.name, FAILLOG)
        cfg += "/bin/su %s -c \"sleep 10 && bash %s/start_resource_monitor.sh &\" - \n" % (USER, TESTDIR)
        cfg += "/bin/su %s -c \"sleep 10 && bash %s/start_neighbor_monitor.sh %s &\" - \n" % ("root", TESTDIR, USER)
