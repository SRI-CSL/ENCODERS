...
"""
Role Bucket Persistence: Tests whether the buckets of encrypted encryption and decryption 
attributes are persisted correctly after restarts.
...
def runTest(env, nodes, results, Console):
    ...
    # Thrice because nodes were shutdown thrice.
    results.expect('Checking whether encryption attribute bucket was persisted exactly twice', 2, BOB.countMatchingLinesInLog( 
                   '{SecurityManager::onPrepareShutdown}: Storing encAttrsBucket'))
    results.expect('Checking whether decryption attribute bucket was persisted exactly twice', 2, BOB.countMatchingLinesInLog( 
                   '{SecurityManager::onPrepareShutdown}: Storing decAttrsBucket'))

    # Once because the attributes should only be there on second boot up
    results.expect('Checking whether encrypted encryption attribute A1 was loaded exactly once', 1, BOB.countMatchingLinesInLog( 
                   '{SecurityManager::onRepositoryData}: Read encrypted encryption attribute: ALICE.A1'))
    results.expect('Checking whether encrypted decryption attribute A2 was loaded exactly once', 1, BOB.countMatchingLinesInLog( 
                   '{SecurityManager::onRepositoryData}: Read encrypted decryption attribute: ALICE.A2'))
