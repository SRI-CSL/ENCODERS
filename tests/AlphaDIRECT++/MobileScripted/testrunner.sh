...
TESTS=$(ls -d ${DIR}/Testsuite/* | sort)
NUM_TESTS=$(ls -d ${DIR}/Testsuite/* | wc -l)
...
for testi in ${TESTS}; do
    ...
    if [ ! -x "${testi}/start_up.sh" ]; then
        echo "INVALID."
        echo "Test file is not specified correctly (no start_up.sh)."
        continue
    fi 
    ...
    if [ "$?" -ne "0" ]; then
        echo "INVALID."
        echo "Could not execute test start up."
        continue 
    fi
    ...
    if [ "$?" -ne "0" ]; then
        echo "INVALID."
        echo "Could not get test location."
        continue 
    fi

    if [ ! -r ${testfile} ]; then
        echo "INVALID."
        echo "Could not read test scenario."
        continue
    fi 

    if [ ! -x "${testi}/tear_down.sh" ]; then
        echo "INVALID."
        echo "Test file is not specified correctly (no tear_down.sh)."
        continue
    fi 

    core -s "${testfile}" &> /dev/null & 
    ...
    if [ "$?" -ne "0" ]; then
        echo "WARNING."
        echo "Test tear down failed, results may be invalid."
    fi
