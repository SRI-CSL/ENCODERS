...
# CBMEN-INTERNAL ONLY. 
#
# Suns-tech work product. June, 2012.
# Contact: sam@suns-tech.com 
#
''' Alpha-DIRECT Basic 3-node Haggle Service
'''
...
class HaggleService(CoreService):
    ...
    def generateconfig(cls, node, filename, services):
        ...
        if node.name == "n1":
            ...
            cfg += "/sbin/ifconfig eth0 broadcast 10.0.0.255\n"
            cfg += "/sbin/ifconfig eth1 broadcast 10.0.1.255\n"
            cfg += "/sbin/route add -net 10.0.0.0 netmask 255.255.255.0 eth0\n"
            cfg += "/sbin/route add -net 10.0.1.0 netmask 255.255.255.0 eth1\n"
        elif node.name ==  "n3":
            cfg += "/sbin/ifconfig eth0 broadcast 10.0.1.255\n"
            cfg += "/sbin/route add default eth0\n"
        cfg += "cp %s ~/.Haggle/config.xml\n" % (CFGFILE,) 
        cfg += "/bin/su - %s -c \"/usr/local/bin/haggle -d -dd -f &\"\n" % (USER,)
