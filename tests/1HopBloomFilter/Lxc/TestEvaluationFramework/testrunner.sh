...
if [ -r "${TEST_FILES}" ]; then
    ...
    do
        ...
        for (( i=1; i<=${NT}; i++ )); do
            TESTS="${TESTS} ${DIR}/${TESTNAME}"
            NUM_TESTS=$((NUM_TESTS+1))
        done
    done < "${TEST_FILES}"
else 
    echo "Loading tests from ${DIR}/Testsuite/*" | tee ${TMPFILE}
...
for testi in ${TESTS}; do
    ...
    if [ "$?" -ne "0" ]; then
        echo "ERROR: Could not stop core." | tee ${TMPFILE}
        exit 1
    fi
    ...
    if [ "$?" -ne "0" ]; then
        echo "ERROR: could not start core." | tee ${TMPFILE}
        exit 1
    fi
